...
class Agent: #Suppose im player 1
    ...
    def simulate(self, board, player, last_pos): # returns 1 if player wins
        ...
        if p1.getWinner(board, last_pos): return 1
        while True:
            # print(board)
    ...
    def train_mcts_once(self, root_node):
        ...
        if len(possible_moves) > 0: # not a terminal node
            ...
            # SIMULATION
            children_wins = [int(self.simulate(deepcopy(child_board), player, (col,row)))
                             for child_board, col, row in zip(children_boards, possible_moves, rows)] #[1,0,1,0,..]
